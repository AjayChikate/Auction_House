...
export default function AuctionRoom() {
  ...
  useEffect(() => {
    ...
    s.on('A_ended', (d) => {
      alert('Auction ended! Winner: ' + (d.highestBid?.bidderName || 'No one'))
      setState((p) => (p ? { ...p, active: false } : p))
    })
    
    s.on('errorMsg', (msg) => {
      if (msg === 'Room not found') setNotFound(true)
      else alert(msg)
    })
  ...
  if (notFound) {
    return <div className="p-6 text-center text-red-400">This auction room doesn't exist or was removed.</div>
  }
  ...
  const sendChat = () => {
    ...
    if (!trimmed) return
    if (trimmed.length > MAX_CHAT_LEN) return alert(`Message too long (max ${MAX_CHAT_LEN} characters)`)
    socketRef.current.emit('chatmessage', { roomId: id, text: trimmed })
    setChatText('')
  }
  ...
  const placeBid = () => {
    ...
    if (!Number.isFinite(amt) || amt <= 0) {
      alert('Bid must be a positive number')
      return
    }
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-900 text-gray-100">
      <div className="flex flex-col lg:flex-row flex-1 gap-6 p-4 md:p-6 relative">
        <div className="w-full lg:w-3/5 flex flex-col space-y-4">
          ...
          <div className="text-base sm:text-lg font-semibold">
            Highest Bid: <span className="text-green-400">₹{state.highestBid?.amount || 0}</span>{' '}
            <span className="ml-1 text-gray-300">by {state.highestBid?.bidderName || 'No one'}</span>
          </div>
          ...
          <div className="flex gap-2 mt-2">
            <input
              ...
              className="border border-gray-700 bg-gray-800 rounded px-3 py-2 flex-1 text-gray-200 focus:outline-none focus:ring-2 focus:ring-green-500 text-sm sm:text-base"
            />
            <button
              onClick={sendChat}
              disabled={!state.active}
              className="flex items-center gap-2 sm:gap-3 bg-gradient-to-r from-green-600 to-green-500 hover:from-green-500 hover:to-green-400 text-white px-3 sm:px-4 py-2 rounded-lg shadow-md transition disabled:opacity-50 text-sm sm:text-base"
            >
          ...
          <div className="flex gap-2 mt-2">
            <input
              ...
              className="border border-gray-700 bg-gray-900 rounded px-3 py-2 flex-1 text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm sm:text-base"
            />
            <button
              onClick={placeBid}
              disabled={!state.active}
              className="flex items-center gap-2 sm:gap-3 bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 text-white px-3 sm:px-4 py-2 rounded-lg shadow-md transition disabled:opacity-50 text-sm sm:text-base"
            >
        ...
        <div className="w-full lg:w-2/5 border border-gray-700 p-4 rounded bg-gray-800 shadow-md flex flex-col space-y-4">
          ...
          <div>
            <h4 className="font-semibold mb-2 text-pink-400">Live Bidding</h4>
            <ul className="space-y-1 max-h-[60vh] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-800 scrollbar-thumb-rounded-full scrollbar-track-rounded-full">
              {(state.bids || []).slice().sort((a, b) => b.amount - a.amount).map((b, i) => (
                <li key={i} className="text-green-300 font-medium">
                  ₹{b.amount} <span className="text-gray-400">by {b.bidderName}</span>
                </li>
              ))}
            </ul>
          </div>
